...
# 0.8.0.XX #
En un futuro lejano. [Si la quieres compilar...](v0_8-Building)
...
* [TMPlayerControl](http://wiki.freepascal.org/TMPlayerControl). También ha de instalarse en el propio Lazarus.
  
Ambos módulos están incluidos como submódulos (de sólo lectura) en GIT, para conocer la versión exacta usada.
...
* [X] Poder filtrar los juegos en base a etiquetas o listas de juegos; estilo CatVer.ini de MAME.
...
* Cambiada, otra vez, la forma de guardar los datos de los juegos y las bases de datos para importar:
  ...
  * Formato CVS (Comma Separated Values), aunque los grupos tienen la extensión .egl. Incluso de pueden abrir con cualquier hoja de cálculo, con su cabecera y todo (aunque no lo recomiendo...).
  * A cambio es mucho más rápida su lectura y escritura (unos 15 segundos para 50000 juegos y 15000 grupos, creo que me repito...).
  * Ya no se usan .INI, para las bases de datos para exportar/importar. Sino CSV también.
...
* Ahora los juegos se pueden filtrar internamente, en vez de tener que haceerlo el GUI.
...
* Se ha quitado el buscar ficheros multimedia que esten de la forma `XXX.zip\NombreJuego.ext`. Es muy lento y entra en conflicto con la forma `NombreJuego.zip\XXX.ext` (Aunque realmente dicha búsqueda solo entraba en funcionamiento si no se encontraban archivos de las otras disponibles). Dicho de otro modo, se busca de las siguiente formas:
  * `NombreJuego.ext`
  * `NombreJuego\XXX.ext`
  * `NombreJuego.zip\XXX.ext`
* ¡Vídeos y Música! usando MPlayer como motor.
* El panel de imagenes, textos, vídeo y música es en cierto modo configurable, pudiendo añadir los subpaneles que se deseen. 
* Se muestran las estadísticas del Sistema y del Emulador.
* El árbol de etiquetas ya funciona y filtra los juegos. Se pueden añadir y quitar grupos seleccionando el juego y pulsando botón derecho sobre el grupo. Cuando se seleccionan varias etiquetas se muestran los juegos que están en todas las etiquetas.
...
* La búsqueda de juegos se debe hacer pulsando el botón "Buscar". Es posiblemente la parte más lenta y que se salte continuamente puede ser molesto.
...
* Implementada la opción de poder asignar archivos en el modo carpeta.
...
De éstas hay algunas cosas que me traen por la calle de la amargura (lo de los
.ico y .png), otras que requieren otro cambio en la estructura (la lentitud) y
otras que son triviales (lo de las bases de datos exportadas).
...
## Formulario principal ##
  ...
  * **Las imágenes de los juegos pueden estar en un archivo comprimido** (o varios :P), funciona en cualquiera de los formatos comprimidos que se reconocen, el nombre del archivo da igual ya que se busca en todos los archivos comprimidos que se encuentren, lo que ya no podría predecir es el orden en el que se recorren... así que es mejor tener solo uno. Tendiendo preferencia el siguiente orden:
    1. Imagen descomprimida del juego
    1. Imagen comprimida del juego
    1. Imagen descomprimida de la familia
    1. Imagen comprimida de la familia
...
# 20080617 - Emuteca 0.3.0.11 #
  ...
  * 
    ...
    * 
      1. Se configura el sistema de recreativas (Juegos de MAME)
      1. Se actualiza la lista
      1. Se crea la base de datos con el propio MAME
      1. Se intenta importar... y no funcionará porque en Emuteca la clave está en mayúsculas y en la base de datos en minúsculas. Por tanto habrá que usar esta opción antes para que funcione bien.
